...
CREATE TABLE `board_members_history` (`board_id` varchar NOT NULL, `user_id` varchar NOT NULL, `action` varchar NULL, `insert_at` datetime NOT NULL DEFAULT (STRFTIME('%Y-%m-%d %H:%M:%f', 'NOW')), PRIMARY KEY (`board_id`, `user_id`, `insert_at`));
...
CREATE TABLE `categories` (`id` varchar NOT NULL, `name` varchar NOT NULL, `user_id` varchar NOT NULL, `team_id` varchar NOT NULL, `channel_id` varchar NULL, `create_at` bigint NULL, `update_at` bigint NULL, `delete_at` bigint NULL, `collapsed` boolean NULL DEFAULT (false), `type` varchar NULL, `sort_order` bigint NULL, PRIMARY KEY (`id`));
...
CREATE TABLE `category_boards` (`id` varchar NOT NULL, `user_id` varchar NOT NULL, `category_id` varchar NOT NULL, `board_id` varchar NOT NULL, `create_at` bigint NULL, `update_at` bigint NULL, `sort_order` bigint NULL, `hidden` boolean NULL, PRIMARY KEY (`id`));
...
CREATE TABLE `file_info` (`id` varchar NOT NULL, `create_at` bigint NOT NULL, `delete_at` bigint NULL, `name` text NOT NULL, `extension` varchar NOT NULL, `size` bigint NOT NULL, `archived` boolean NULL, `path` varchar NULL);
...
CREATE TABLE `preferences` (`userid` varchar NOT NULL, `category` varchar NOT NULL, `name` varchar NOT NULL, `value` text NULL, PRIMARY KEY (`userid`, `category`, `name`));
...
-- Where a card's branch is published. Was the `deploys` array in
-- config.json, and a route's stage named it by name.
CREATE TABLE `deploy_target` (`id` varchar(36) NOT NULL, `name` varchar(100) NOT NULL, `ssh_host` varchar(255) NOT NULL, `ssh_user` varchar(64) NULL, `ssh_port` bigint NULL, `ssh_key` text NULL, `base_app` varchar(100) NULL, `base_domain` varchar(255) NULL, `created_at` bigint NOT NULL, PRIMARY KEY (`id`), UNIQUE INDEX `idx_deploy_target_name` (`name`)) CHARSET utf8mb4 COLLATE utf8mb4_general_ci;
...
--     attachment id was truncated by one. UUIDv7 needs 36, which is what
--     every other id column already holds.
CREATE TABLE `file_info` (`id` varchar(36) NOT NULL, `create_at` bigint NOT NULL, `delete_at` bigint NULL, `name` text NOT NULL, `extension` varchar(50) NOT NULL, `size` bigint NOT NULL, `archived` bool NULL, `path` varchar(512) NULL) CHARSET utf8mb4 COLLATE utf8mb4_general_ci;
...
--     Still stored in the clear here, exactly as it was in the settings
--     file: moving it into internal/secrets is its own change.
CREATE TABLE `proxy` (`id` varchar(36) NOT NULL, `name` varchar(100) NOT NULL, `url` text NULL, `no_proxy` text NULL, `ca_cert` text NULL, `username` varchar(100) NULL, `password` text NULL, `created_at` bigint NOT NULL, PRIMARY KEY (`id`), UNIQUE INDEX `idx_proxy_name` (`name`)) CHARSET utf8mb4 COLLATE utf8mb4_general_ci;
...
--     terminalCommand, mcpServers. JSON on purpose: this is how to start
--     a process, it points at nothing and nothing joins to it.
CREATE TABLE `agent` (`id` varchar(36) NOT NULL, `name` varchar(100) NOT NULL, `kind` varchar(32) NOT NULL, `user_id` varchar(36) NULL, `proxy_id` varchar(36) NULL, `bin_path` text NULL, `model` varchar(100) NULL, `prompt` text NULL, `settings` text NULL, `created_at` bigint NOT NULL, PRIMARY KEY (`id`), UNIQUE INDEX `idx_agent_name` (`name`), CONSTRAINT `agent_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE SET NULL, CONSTRAINT `agent_proxy` FOREIGN KEY (`proxy_id`) REFERENCES `proxy` (`id`) ON DELETE SET NULL) CHARSET utf8mb4 COLLATE utf8mb4_general_ci;
...
--     terminal. The rest have no button, which is why this is a field and
--     never a reading of the reason's own Russian.
CREATE TABLE `card_stall` (`card_id` varchar(36) NOT NULL, `node_id` varchar(64) NULL, `kind` varchar(32) NULL, `reason` text NOT NULL, `created_at` bigint NOT NULL, PRIMARY KEY (`card_id`), CONSTRAINT `card_stall_card` FOREIGN KEY (`card_id`) REFERENCES `blocks` (`id`) ON DELETE CASCADE) CHARSET utf8mb4 COLLATE utf8mb4_general_ci;
...
--     'git' means a repository was demanded and one without git is an
--     error. NULL is the ordinary case and means nobody said.
CREATE TABLE `workspace` (`id` varchar(36) NOT NULL, `name` varchar(200) NOT NULL, `path` text NULL, `board_id` varchar(36) NULL, `global` bool NOT NULL, `kind` varchar(16) NULL, `base_branch` varchar(200) NULL, `branch_prefix` varchar(64) NULL, `created_at` bigint NOT NULL, PRIMARY KEY (`id`), UNIQUE INDEX `idx_workspace_name` (`name`), CONSTRAINT `workspace_offered_on` FOREIGN KEY (`board_id`) REFERENCES `boards` (`id`) ON DELETE SET NULL) CHARSET utf8mb4 COLLATE utf8mb4_general_ci;
...
--   workdir_path: Was repo_path. Becomes workspace_id in the second half of step 2,
--     together with the code that resolves a folder to its registry id.
CREATE TABLE `conversation` (`id` varchar(36) NOT NULL, `card_id` varchar(36) NULL, `board_id` varchar(36) NULL, `node_id` varchar(64) NOT NULL, `column_name` varchar(200) NULL, `title` text NULL, `summary` text NULL, `agent` varchar(100) NULL, `kind` varchar(32) NULL, `workdir_path` text NULL, `cwd` text NULL, `branch` varchar(255) NULL, `started_at` bigint NOT NULL, `ended_at` bigint NULL, `exit_code` bigint NULL, PRIMARY KEY (`id`), INDEX `idx_conversation_card` (`card_id`, `started_at`), INDEX `idx_conversation_card_node` (`card_id`, `node_id`, `started_at`), CONSTRAINT `conversation_card` FOREIGN KEY (`card_id`) REFERENCES `blocks` (`id`) ON DELETE CASCADE, CONSTRAINT `conversation_board` FOREIGN KEY (`board_id`) REFERENCES `boards` (`id`) ON DELETE CASCADE) CHARSET utf8mb4 COLLATE utf8mb4_general_ci;
...
-- "as it happened" and the three spellings of AUTO_INCREMENT are gone.
-- `seq` went with them — it was the same fact told twice.
CREATE TABLE `session_event` (`id` varchar(36) NOT NULL, `session_id` varchar(36) NOT NULL, `kind` varchar(32) NOT NULL, `payload_json` text NULL, `created_at` bigint NOT NULL, PRIMARY KEY (`id`), INDEX `idx_session_event_session` (`session_id`, `id`), CONSTRAINT `session_event_session` FOREIGN KEY (`session_id`) REFERENCES `agent_session` (`id`) ON DELETE CASCADE) CHARSET utf8mb4 COLLATE utf8mb4_general_ci;
...
--   version: The service's own updated/etag: changed means there is something
--     to add.
CREATE TABLE `source_item` (`source` varchar(100) NOT NULL, `external_id` varchar(255) NOT NULL, `version` varchar(255) NULL, `card_id` varchar(36) NULL, `created_at` bigint NOT NULL, `updated_at` bigint NOT NULL, PRIMARY KEY (`source`, `external_id`), INDEX `idx_source_item_card` (`card_id`), CONSTRAINT `source_item_card` FOREIGN KEY (`card_id`) REFERENCES `blocks` (`id`) ON DELETE SET NULL) CHARSET utf8mb4 COLLATE utf8mb4_general_ci;
...
-- latches, and MySQL would refuse a path in a key of any real length.
--   marker: The commit the event refers to: the same state seen twice fires once.
CREATE TABLE `vcs_seen` (`workspace_id` varchar(36) NOT NULL, `branch` varchar(255) NOT NULL, `kind` varchar(32) NOT NULL, `marker` varchar(64) NULL, `created_at` bigint NOT NULL, PRIMARY KEY (`workspace_id`, `branch`, `kind`), CONSTRAINT `vcs_seen_workspace` FOREIGN KEY (`workspace_id`) REFERENCES `workspace` (`id`) ON DELETE CASCADE) CHARSET utf8mb4 COLLATE utf8mb4_general_ci;
...
-- reserved word in MySQL, so it is `token` — cheaper than backticks in
-- every query for ever.
CREATE TABLE "idempotency" ("token" character varying(255) NOT NULL, "session_id" character varying(36) NULL, "created_at" bigint NOT NULL, PRIMARY KEY ("token"));
...
-- Who joined or left a board, and when.
CREATE TABLE "board_members_history" ("board_id" character varying(36) NOT NULL, "user_id" character varying(36) NOT NULL, "action" character varying(10) NULL, "insert_at" timestamptz NOT NULL DEFAULT NOW(), PRIMARY KEY ("board_id", "user_id", "insert_at"));
...
-- Which board sits in which sidebar group. The surrogate id is
-- upstream's; the row is really the pair, which the unique index says.
CREATE TABLE "category_boards" ("id" character varying(36) NOT NULL, "user_id" character varying(36) NOT NULL, "category_id" character varying(36) NOT NULL, "board_id" character varying(36) NOT NULL, "create_at" bigint NULL, "update_at" bigint NULL, "sort_order" bigint NULL, "hidden" boolean NULL, PRIMARY KEY ("id"));
...
--     attachment id was truncated by one. UUIDv7 needs 36, which is what
--     every other id column already holds.
CREATE TABLE "file_info" ("id" character varying(36) NOT NULL, "create_at" bigint NOT NULL, "delete_at" bigint NULL, "name" text NOT NULL, "extension" character varying(50) NOT NULL, "size" bigint NOT NULL, "archived" boolean NULL, "path" character varying(512) NULL);

-- What a person chose, per category and name. Not where this app keeps
-- the page's own settings — those are <dataDir>/ui-settings.json, because
-- the desktop window opens on a fresh origin every launch.
CREATE TABLE "preferences" ("userid" character varying(36) NOT NULL, "category" character varying(32) NOT NULL, "name" character varying(32) NOT NULL, "value" text NULL, PRIMARY KEY ("userid", "category", "name"));
...
--     Still stored in the clear here, exactly as it was in the settings
--     file: moving it into internal/secrets is its own change.
CREATE TABLE "proxy" ("id" character varying(36) NOT NULL, "name" character varying(100) NOT NULL, "url" text NULL, "no_proxy" text NULL, "ca_cert" text NULL, "username" character varying(100) NULL, "password" text NULL, "created_at" bigint NOT NULL, PRIMARY KEY ("id"));
...
--     terminalCommand, mcpServers. JSON on purpose: this is how to start
--     a process, it points at nothing and nothing joins to it.
CREATE TABLE "agent" ("id" character varying(36) NOT NULL, "name" character varying(100) NOT NULL, "kind" character varying(32) NOT NULL, "user_id" character varying(36) NULL, "proxy_id" character varying(36) NULL, "bin_path" text NULL, "model" character varying(100) NULL, "prompt" text NULL, "settings" text NULL, "created_at" bigint NOT NULL, PRIMARY KEY ("id"), CONSTRAINT "agent_user" FOREIGN KEY ("user_id") REFERENCES "users" ("id") ON DELETE SET NULL, CONSTRAINT "agent_proxy" FOREIGN KEY ("proxy_id") REFERENCES "proxy" ("id") ON DELETE SET NULL);
...
--     terminal. The rest have no button, which is why this is a field and
--     never a reading of the reason's own Russian.
CREATE TABLE "card_stall" ("card_id" character varying(36) NOT NULL, "node_id" character varying(64) NULL, "kind" character varying(32) NULL, "reason" text NOT NULL, "created_at" bigint NOT NULL, PRIMARY KEY ("card_id"), CONSTRAINT "card_stall_card" FOREIGN KEY ("card_id") REFERENCES "blocks" ("id") ON DELETE CASCADE);
...
--     'git' means a repository was demanded and one without git is an
--     error. NULL is the ordinary case and means nobody said.
CREATE TABLE "workspace" ("id" character varying(36) NOT NULL, "name" character varying(200) NOT NULL, "path" text NULL, "board_id" character varying(36) NULL, "global" boolean NOT NULL, "kind" character varying(16) NULL, "base_branch" character varying(200) NULL, "branch_prefix" character varying(64) NULL, "created_at" bigint NOT NULL, PRIMARY KEY ("id"), CONSTRAINT "workspace_offered_on" FOREIGN KEY ("board_id") REFERENCES "boards" ("id") ON DELETE SET NULL);
...
-- "as it happened" and the three spellings of AUTO_INCREMENT are gone.
-- `seq` went with them — it was the same fact told twice.
CREATE TABLE "session_event" ("id" character varying(36) NOT NULL, "session_id" character varying(36) NOT NULL, "kind" character varying(32) NOT NULL, "payload_json" text NULL, "created_at" bigint NOT NULL, PRIMARY KEY ("id"), CONSTRAINT "session_event_session" FOREIGN KEY ("session_id") REFERENCES "agent_session" ("id") ON DELETE CASCADE);
...
--   version: The service's own updated/etag: changed means there is something
--     to add.
CREATE TABLE "source_item" ("source" character varying(100) NOT NULL, "external_id" character varying(255) NOT NULL, "version" character varying(255) NULL, "card_id" character varying(36) NULL, "created_at" bigint NOT NULL, "updated_at" bigint NOT NULL, PRIMARY KEY ("source", "external_id"), CONSTRAINT "source_item_card" FOREIGN KEY ("card_id") REFERENCES "blocks" ("id") ON DELETE SET NULL);
