...
## ~~Что нашло ревью ветки (2026-08-18)~~ — все три находки закрыты

Сверка плана с реализацией до слияния нашла три вещи; ни одна не дожила до
main.

- **MySQL-миграция не исполнялась** — atlas рендерил `DEFAULT "NOW(6)"`
  строкой. Починено вместе с прогоном на трёх базах («Run the tests on all
  three databases…»): `render.go` говорит `CURRENT_TIMESTAMP(6)`, и эта
  находка оказалась лишь первой из шести, которые принёс первый настоящий
  прогон, — остальные пять и сам прогон описаны в `docs/sql-plan.md`.
- **Без тега `sqlite3` внешние ключи были молча выключены** —
  `internal/appschema` импортировал mattn безусловно, а прагму писал по
  выбранному тегом драйверу, так что бестеговый `go test ./...` ронял три
  `cascade_test`. Починено в усушке («Give appschema the DSN its own driver
  understands»): прагма пишется под драйвер, который пакет сам импортирует.
- **Хвост шага 3, `triggerProperty`** — колонку карточки называл
  `config.json`, и имя ехало живым в `BoardGrant.Property`. Починено там же
  («Take the agent's column property from the board, not from the
  settings»): грант спрашивает свойство у доски (`xciiiColumnProperty`), как
  это всегда делал `boardadapter.ColumnProperty`. В `flowtemplates` то же имя
  остаётся законно — это seed на момент создания доски.
...
2. ~~**Реестры перестают быть JSON**~~ — сделано 2026-08-17: папки, агенты,
   прокси, деплой-цели — таблицы с `id`. Закрыло противоречия 2, 3, 8.
3. ~~**`config.json` перестаёт называть колонки и пути**~~ — сделано
   2026-08-18: свойство колонок записывает доска, по id; хвост
   (`triggerProperty` в гранте) добит в усушке. Закрыло 1, 6, 9.
...
- **если три диалекта — цель**, наш стор переезжает на squirrel с тем же
  ...
  ручными в обоих мирах. Прогон на докерных MySQL/Postgres с тех пор появился
  (`internal/dbtest`, `wails3 task test:db:all`) и `DEFAULT "NOW(6)"` починен
  вместе с ним, так что из цены осталась только сама правка — ~50 методов
  механической; инвентарь того, что в запросах форка ещё зависит от вендора,
...
Пока не решено, по факту действует второе — с оговоркой, которой не было:
DDL и запросы форка теперь проверяются на трёх вендорах, а запросы наших
таблиц по-прежнему исполняются на одном. Решить стоит сознательно, а не по
накоплению.
