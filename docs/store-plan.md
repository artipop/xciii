...
0. ~~**Схлопнуть миграции в одну**~~ — сделано: один `000001_init`, вся схема,
   ...
   вторая половина: **переназначить типы** — id в UUIDv7, время в настоящий
   timestamp, **у нас и у доски одновременно**, потому что MySQL не даст
   внешнего ключа между колонками разных типов.
