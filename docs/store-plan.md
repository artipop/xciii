...
**Чем хранить — решение с ценой.** Двоичные 16 байт быстрее и меньше, но база
десктопного приложения — это файл, который открывают `sqlite3` и смотрят
глазами (из чего, собственно, написан `docs/db-erd.md`), а blob'ы это убивают.
Поэтому по умолчанию — **каноническая текстовая форма, `VARCHAR(36) CHARACTER
SET ascii`** (ascii важен: в utf8mb4 MySQL считает по четыре байта на символ и
съедает предел длины ключа), и native `uuid` на Postgres, который читаемый и
так. Если однажды упрёмся в размер — переход на `BINARY(16)` прячется в одном
Go-типе со `Scan`/`Value` и схему больше нигде не трогает.
...
вместе с переходом на UUIDv7 в `VARCHAR(36) CHARACTER SET ascii`; короткие
технические строки (`node_id`, `kind`, `status`, `mode`) — `VARCHAR(n)`,
свободный текст (`title`, `summary`, `said`, `detail`) остаётся `TEXT`.

**И следом — предел длины ключа.** У InnoDB это 3072 байта, а в utf8mb4 символ
