...
`internal/sources/store.go` больше не открывают файлов: `NewStore(db)`
...
Ключи шаг 1 написал, но не включил; включены они в шаге 4 (ниже) — DSN, а не
`PRAGMA`, потому что это настройка соединения.

## Что нашло ревью ветки (2026-08-18)

Сверка плана с реализацией, до слияния. Три находки; первая отложена
сознательно, две другие — правки на полчаса каждая.

**MySQL-миграция не исполнится.** В `000001_init.up.sql` у history-таблиц
стоит `insert_at datetime(6) NOT NULL DEFAULT "NOW(6)"`: в MySQL двойные
кавычки — строковый литерал, а строка не бывает DEFAULT у datetime, так что
`CREATE TABLE` упадёт на первой же history-таблице. SQLite (`STRFTIME(…)`) и
Postgres (`DEFAULT NOW()`) отрендерены верно — сломан ровно тот диалект,
который здесь нечем запустить. Лечение известно: `CURRENT_TIMESTAMP(6)`
вместо `NOW(6)` в `render.go` (MySQL принимает его как DEFAULT без скобок
выражения) и один прогон миграции на докерных MySQL и Postgres, чтобы
«проходит на всех трёх» стало фактом, а не свойством генератора. Отложено:
чинится тогда же, когда решится вопрос про диалекты (ниже, «Чем говорить с
базой»).

**Без тега `sqlite3` внешние ключи молча выключены.** `internal/appschema`
импортирует mattn безусловно, а `SQLiteDSN` выбирается тегом сборки: в сборке
без тега прагма пишется по-modernc (`_pragma=foreign_keys(1)`), mattn её
молча игнорирует, и каскад не срабатывает — три `cascade_test` падают в
`go test ./...` и зелёные с `-tags "json1 sqlite3"`. Это ровно тот тихий
отказ, от которого шаг 4 защищался, только в тестовой сборке. Починка:
`appschema` пишет прагму под драйвер, который сам же импортирует
(`_foreign_keys=on` жёстко), — либо импортирует драйвер под тем же тегом, что
и стор, чтобы обе половины выбирались одним переключателем.

**Хвост шага 3: `triggerProperty` остался.** Ключ по-прежнему сериализуется в
`config.json` (по умолчанию «Статус») и едет живым в `BoardGrant.Property`:
инструменты доски читают и пишут колонку карточки по имени свойства из
машинного файла — настройки машины называют кусок чужой доски, ровно то, о
чём шаг 3. Доска своё свойство уже записывает (`xciiiColumnProperty`, по id);
грант должен спрашивать её, как это делает `boardadapter.ColumnProperty`. В
`flowtemplates` то же имя — seed на момент создания доски, это законно и
остаётся.
...
4. ~~**Включить внешние ключи**~~ — сделано: `foreign_keys` в DSN, каскад от
   карточки работает. Осталась разовая уборка висячих ссылок, накопленных до.
0. ~~**Схлопнуть миграции в одну**~~ — сделано: один `000001_init`, вся схема,
   и тест, который сверяет её со снимком того, что строила лестница. Осталась
   вторая половина: **переназначить типы**. Id уже UUIDv7 текстом с обеих
   сторон (2026-08-18); осталось время в настоящий timestamp, **у нас и у
   доски одновременно**, потому что MySQL не даст внешнего ключа между
   колонками разных типов, — и бинарь для id, если он переживёт пересмотр в
   разделе про идентификаторы.
...
**Пересмотр 2026-08-18: бинарь, похоже, не нужен.** UUIDv7 текстом уже сделан
с обеих сторон (`utils.NewID`, `Utils.createGuid`): упорядоченность и
настоящую уникальность он уже дал, а каноническая форма сортируется ровно как
байты. Из выигрышей выше остаётся один — колонка, отказывающая мусору, — и
его даёт `CHECK` по форме дешевле, чем смена типа хранения: без конвертора на
каждой границе и без вьюхи на каждую таблицу. Экономия места и локальность
индекса на однопользовательском SQLite не окупятся никогда. Рекомендация:
текст в 36 везде, бинарь вычеркнуть; native `uuid` в Postgres — тогда, когда
Postgres станет диалектом, на котором это приложение правда запускают (см.
«Чем говорить с базой»). Хуже всего — нести бинарь как долг «потом
переведём».
...
**~~Перечисления — `CHECK`~~ — сделано 2026-08-18**, но не на всех пяти
колонках, и это правило, а не недоделка. Проверка стоит там, где множество
закрыто **моделью**: `checkout.mode` и `workspace_board.mode` (три способа
работать с папкой, четвёртого нет), `agent_session.status` (жизненный цикл),
`board_setup.status`, `source_event.outcome`. Её нет там, где множество —
список, который растёт: `agent.kind` — это вендорский CLI, `session_event.kind`
— имя события, `workspace.kind` должен со временем перерасти `git` и стать
диском или машиной по ssh. SQLite не умеет `ALTER` для check'а, так что
проверка на растущем множестве покупается пересозданием таблицы при каждом
новом значении — цена выше, чем опечатка, которую она ловит.

Ловит она и разницу в регистре: `'RUNNING'` отказывается так же, как
`'runnnig'` (`TestAValueOutsideAClosedSetIsRefused`). Работает во всех трёх
(MySQL — с 8.0.16); `Check` в `tools/schemagen/types.go` дописывает
`col IS NULL OR` для nullable-колонки, иначе check, ложный для NULL, отказал бы
каждой строке, которая колонку не заполнила.
...
**~~Мёртвое — убрать~~ — частично, 2026-08-18.** Ушли четыре колонки и код,
который их писал: `root_id` у `blocks` и `blocks_history` (всегда равен
`board_id`, писал его только `legacy_blocks.go` — 206 строк, которые не звал
никто) и `workspace_id` у `sharing`, `subscriptions`, `notification_hints`
(остаток древнее `channel_id`: workspace'ы были до команд, и ни один запрос не
называл его годами). С ними ушли `replaceBlockID` — тоже без вызовов — и
`Block.WorkspaceID`.

Остались `team_id`, всегда равный `'0'`, и `blocks.channel_id`. Первый ещё
читается (`GlobalTeamID` ходит по коду доски), у второго есть живой путь в
`app/boards.go` и поле в API, которое видит страница; снять их — это снять
работу с командами и привязку доски к каналу, а не почистить схему.
...
## Чем говорить с базой: ORM не нужен, нужны два типа и один строитель

Вопрос встал из объёма: 154 упоминания `create_at`/`update_at` в сторе форка,
31 `UnixMilli` у нас — кажется, что нужен слой, который абстрагирует время и
конвертирует его в Go-тип. Такой слой есть, и это не ORM, а стандартная
библиотека: `sql.Scanner` и `driver.Valuer`. Тип поверх `time.Time`, который
знает, как он хранится, — и `database/sql` конвертирует на границе сам, под
squirrel и под голым SQL одинаково. Поле модели меняет тип — и большинство из
тех 154 упоминаний не правки вовсе: это имена колонок в селектах, которые не
меняются, и `&block.CreateAt` в сканах, которые не меняются тоже. Правится
арифметика (`GetMillis()` и сравнения) и граница API, где время по-прежнему
уходит наружу числом (`MarshalJSON` у того же типа). ORM не добавил бы к
этому ничего, чего не даёт пакет типов на полсотни строк.

**Что действительно разъехалось на границе форк/наше — не строитель, а
диалекты.** Форк говорит на трёх: squirrel с `PlaceholderFormat` и ручные
`ON CONFLICT`/`ON DUPLICATE KEY` там, где upsert, — потому что upsert не
решает ни squirrel, ни большинство ORM. Наш стор говорит на одном: голый
`database/sql`, `?` (Postgres не понимает) и голый `ON CONFLICT` (MySQL не
понимает). То есть DDL сегодня обещает три диалекта, а запросы наших таблиц
исполняются на одном — обещание схемы шире обещания кода. Развилка:

- **если три диалекта — цель**, наш стор переезжает на squirrel с тем же
  builder'ом, что у форка (это и есть «везде squirrel»), upsert-ы остаются
  ручными в обоих мирах, и появляется прогон на докерных MySQL/Postgres —
  тогда же чинится и `DEFAULT "NOW(6)"`. Цена — ~50 методов механической
  правки;
- **если цель — SQLite**, это записывается словами, рендер mysql/pg остаётся
  опцией на вырост, и «нигде» — тоже честный ответ: голому SQL на одном
  диалекте строитель не нужен.

Пока не решено, по факту действует второе; решить стоит сознательно, а не
по накоплению.

**Схлопывание границы, которое окупается при любом ответе, — типы, не
строитель.** Один пакет уровня модуля, который импортируют обе половины:
момент времени со `Scan`/`Value`/`MarshalJSON`, знающий своё хранение, — и он
же словарь `tools/schemagen`, чтобы тип колонки в схеме и тип поля в Go были
одним решением, записанным один раз. Тогда «одинаково везде» — это не один
query builder, а одно место, где сказано, как время лежит в базе и как оно
выглядит в Go; строители могут различаться, типы — нет.

**ent: вердикт прежний, условие пересмотра — новое.** ent не сокращает правку
типов, а замещает её переписыванием всех ~130 query-сайтов форка, включая
history-`INSERT … SELECT` и undelete, — отдельный проект без видимой
пользователю пользы (`docs/schema/ent.md` — разбор). Переоткрыть вопрос стоит
один раз: **после усушки форка**. Доставка нотификаций уже удалена; подписки,
комплаенс, шаринг — кандидаты той же судьбы. Когда останется та половина
стора, которую это приложение правда зовёт, переписать её — во что угодно —
станет проектом обозримого размера. До того — нет.

## ~~Схлопнуть 81 миграцию в одну~~ — сделано; типы остались

> Шаг 0 разделился надвое, и разделился по линии «что можно проверить». Схлопывание
> **сделано** 2026-08-17: вместо восьмидесяти одного файла — один `000001_init`,
> и в нём вся схема, форковая и наша. Переназначение типов (UUIDv7, настоящий
> timestamp) — **нет**: это правка бордового стора, webapp и history-таблиц
> одним куском, и она едет отдельно.
> К 2026-08-18 она усохла: id уже UUIDv7 текстом с обеих сторон, так что
> неделимым осталось время — и бинарь, только если он переживёт пересмотр в
> разделе про идентификаторы.

**Чем это проверено, и почему проверка тут — главное.** Вся ставка схлопывания в
том, что одна миграция строит ту же схему, что строили восемьдесят одна. Это
проверяется, а не предполагается: `go run ./tools/schemagen -collapsed-sqlite`
печатает схему обычным SQLite-DDL, и она сравнивается с базой, которую построила
лестница, — по колонкам, типам, NOT NULL, значениям по умолчанию, первичным
ключам и по тому, что покрывает каждый индекс. Снимок дореформенной схемы лежит
в `tools/schemagen/testdata/`, потому что саму лестницу из дерева уже не собрать.

Сравнение **по смыслу, а не по тексту**: SQLite показывает UNIQUE-ограничение
безымянным автоиндексом, а `CREATE UNIQUE INDEX` — под его собственным именем,
и это одно и то же. Проверено, что проверка ловит: выброшенная колонка и
изменённая ширина обе валят тест.

**Ширины пришлось проверять отдельно**, и это стоит знать: SQLite их
игнорирует, поэтому atlas их туда и не печатает — то есть `varchar(64)`,
ставший `varchar(32)`, схемным сравнением **не** ловится. На MySQL и Postgres
это молча режет чужие данные, а запустить их здесь нечем. Отдельный тест сверяет
ширины с тем же снимком.

**Чего схлопывание не сделало, намеренно.** Внешних ключей между бордовыми
таблицами нет: форк удаляет мягко (`delete_at`), и настоящий ключ сработал бы на
тех путях, где удаление настоящее, — это изменение поведения под кодом, который
ради этого никто не читал. Мёртвые колонки (`channel_id`, `workspace_id`,
`root_id`) и отсутствующий у `file_info` первичный ключ **воспроизведены как
есть**: смысл схлопывания в том, что результат сверяем, а улучшения приезжают
потом и поодиночке, каждое своим разбором.

**База от старой лестницы не открывается**, и это решение: она записана версией
41, а такого шага больше нет, и вместо «no migration found for version 41» она
получает внятный отказ с тем, что делать. Умение принять её (переставить версию
на 1 — схема-то та же) было написано и выброшено: релиза нет, такие базы есть
только у тех, кто это и пишет, а константа соответствия версий, которую пришлось
бы нести вечно, — ровно та археология, которой здесь только что стало на
восемьдесят один файл меньше.

Дальше — исходный разбор, по которому это делалось.
...
под ним; `runServerWithLogger` заворачивает их в `board{srv, db}`, и
...
**Префикса у таблиц больше нет.** Имена таблиц писались `{в фигурных скобках}`
и раскрывались `Store.sql` — потому что бордовые таблицы несли `s.tablePrefix`,
а тот существовал ради одного: Focalboard жил плагином внутри базы Mattermost и
занимал в ней своё пространство имён. Это приложение префикс не задаёт никогда
(`DBTablePrefix` по умолчанию `""`), а два места, которые задавали, — тесты, и
им всё равно достаётся отдельная база. Убрано целиком: `{{.prefix}}` из
миграции, `s.tablePrefix` из ста тридцати запросов, поле из конфига, скобки из
наших. Вместе с ним ушли хелперы, ради которых миграции вообще рендерились
шаблоном (`addColumnIfNeeded`, `doesColumnExist`, `renameTableIfNeeded` и ещё
шесть) — четыреста строк подиалектной сборки SQL, которые схлопнутая лестница не
зовёт ниоткуда. В миграции остались только три `{{if .sqlite}}`.
...
## ~~Шаг 2.~~ Реестры стали таблицами

Сделано 2026-08-17. Ниже — то, что план обещал; здесь то, чем оно оказалось.

**Реестры.** `workspace`, `workspace_board`, `agent`, `proxy`, `deploy_target`.
База — правда, `Config` держит те же записи слайсом как кэш (движок читает их на
каждом переезде карточки), всё пишется насквозь. `config.json` хранит копию
ключей ещё одну версию — это путь отката и ничего больше.

**`workdir_claim` стал `checkout`**, с ключом на `workspace.id` (RESTRICT: папку,
в которой карточка работает, не удалить) и владельцем, разложенным на `card_id`
и `board_id`, — одна колонка не может смотреть на две таблицы. **Закрывает
противоречие 3.** `vcs_seen` туда же.

**Чего перелив не переносит — и это единственное, что он выбрасывает
сознательно:** старые `workdir_claim` и `vcs_seen`. Они называют папку путём, а
таблицы ключуются id, и реестра, который переводит одно в другое, на момент
перелива ещё нет: он читается позже, когда стартует менеджер. Отложить их до
того момента — это второй перелив со своим порядком, который тоже можно
перепутать, ради данных, которые восстанавливаются сами: ветка лежит на карточке
и в git, копия — на диске, а имя ветки не помнится, а **выводится** из карточки и
владельца, так что при следующем обращении вычислится то же имя и подхватится тот
же worktree. Теряется запись «эта папка занята», так что карточка, державшая
папку в режиме `branch`, перестаёт её держать до следующего обращения; и одна
git-защёлка может сработать второй раз на первом опросе.

**Чего в плане не было, а пришлось.** Папка, на которую карточка указала путём
(`repo_path`, противоречие 6), в реестре не значится — а `checkout` обязан
назвать workspace. Молча не записать было нельзя: в режиме `branch` именно запись
и держит папку, так что вторая карточка вошла бы в рабочую копию первой. Поэтому
такая папка **регистрируется** — под именем своего каталога и без доски
(«не привязана ни к какой доске» — состояние, которое у продукта уже есть).
Уходит вместе с `repo_path` в шаге 3.

**Чего шаг 2 не сделал.** Составы колонок по-прежнему хранят **имена** агентов и
целей, а не id (противоречия 2 и 8 остаются). Поля `conversation.workdir_path` и
`flow_state.workdir_path` тоже остались путями: это диагностика и подпись, а не
идентичность, и переписывать их отдельной миграцией незачем — схема всё равно
пишется заново в шаге 0.

## Шаг 2 (как он был задуман)
...
## ~~Шаг 3.~~ `config.json` перестал называть колонки и пути

Сделано 2026-08-18.

**Что ушло из файла.** Семь ключей — `projects`, `agents`, `proxies`,
`deploys`, `columns`, `flows`, `boardPrompts`. Первые четыре теперь таблицы,
последние три — свойства доски (`xciiiColumns`, `xciiiFlows`, `xciiiPrompt`).
Поля в `Config` остались, но помечены `json:"-"`: это рабочая копия, которую
движок читает на каждом переезде карточки, а заполняется она из базы
(`loadRegistriesLocked`) и с досок (`SeedBoard`). Копия «ещё на одну версию как
путь отката», о которой говорил шаг 2, — это была она; версия наступила.

Вместе с ключами ушло то, что их читало: `moveAutomationToBoards` (разовый
перенос автоматики из файла на доски) и `configToStore` с картой `boardStored`
(они решали, что ещё писать в файл, — теперь не пишется ничего). Тесты, которые
проверяли круг «сохранили → прочитали config.json», ходят через базу:
`reloaded(t, m)` — второй менеджер на той же базе, то есть следующий запуск.

**Токен.** `githubToken` из файла настроек ушёл в `internal/secrets`, под ключ
`github.token`: это учётка, а лежала она между таймаутами, в файле, который
правят руками и вставляют в issue. Порядок чтения прежний — окружение
(`GITHUB_TOKEN`), потом хранилище. Хранилище теперь строится всегда, а не внутри
блока источников: агентная половина тоже его читает, и одна не подразумевает
другую.

**Что в файле осталось** — ровно то, на что никто не ссылается: собственные
каталоги (`worktreeDir`, `artifactsDir`), пределы и таймауты (`maxConcurrent`,
`testTimeoutMinutes`, `vcsPollSeconds`), переключатели (`enabled`,
`agentNamedBranches`, `worktreeMode` как умолчание) и тексты промптов по
умолчанию. Файл остался тем, чем он полезен, — местом, которое правят руками.

Колонки по id вместо имени и снятый `ProjectWhitelist` приехали раньше, своими
изменениями (противоречия 1 и 6).

## ~~Шаг 4.~~ Внешние ключи включены

Сделано 2026-08-17. `foreign_keys` — настройка **соединения**, поэтому она в DSN,
а не в миграции: `PRAGMA`, выполненный один раз, держится для одного соединения
и молча выключен на следующем. Пул на SQLite и так ограничен одним соединением
(`server.NewStore`), так что pragma бы удержалась — но тогда настройка пула стала
бы несущей для целостности, а это ловушка для того, кто пул расширит.

Имя параметра зависит от драйвера, а драйвер выбирается тегом сборки, поэтому
`SQLiteDSN` — функция с двумя реализациями рядом с импортом, который драйвер и
выбирает (`sqlite_dsn_cgo.go`, `sqlite_dsn_pure.go`): `_foreign_keys=on` у cgo,
`_pragma=foreign_keys(1)` у modernc. Она же аккуратно доклеивает параметры к
DSN, в котором уже что-то есть, — на этом сразу поймался тест, чей DSN нёс
`?_busy_timeout=5000`.

**Что теперь правда.** Удаление карточки забирает её разговоры, позицию на
маршруте, стоп-запись и место в очереди. Журнал источника — исключение и
намеренно: что решил источник, это факт об источнике, поэтому строка остаётся, а
ссылка обнуляется. Папку, в которой карточка работает, удалить нельзя (RESTRICT),
и она освобождается, когда карточка уходит.

**Чем проверено.** `internal/acp/cascade_test.go` — на базе, открытой тем же
`SQLiteDSN`, которым её открывает приложение, чтобы тест не мог пройти по
правилу, которого приложение не применяет. И на живой базе: 17 досок, 444 блока,
`PRAGMA foreign_key_check` чист, удаление карточки уносит стоп-запись.

**Чего это не делает.** SQLite проверяет ключи при записи, а не при включении, —
так что строка, повисшая до этого, лежит и не мешает, пока её не тронут. Разовую
уборку (в первую очередь `workspace.board_id` доски, которую удалили, пока
приложение не работало) стоит сделать отдельно и видимо; молча переписывать чужой
реестр по догадке — хуже, чем висячая ссылка, которая никому не мешает.

**Тесты приложения по-прежнему открывают базу без проверки** (`appschema.Open`), и
это решение, а не недоделка: фикстуры называют карточки «card-1», а доски
«board1», которых ни на одной доске не было, и заставить каждую сначала вставить
строку в `blocks` — значит превратить каждый тест в маленькую модель доски и
тестировать модель. Само правило проверяется там, где оно и должно, —
`appschema.OpenEnforcing`.
...
| 0 ~~схлопнуть~~ (сделано) и переназначить типы | 1500–2500 | — | ~200 |
