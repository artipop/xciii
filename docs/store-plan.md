...
0. **Схлопнуть миграции в одну** — релиза не было, а `ALTER TABLE ADD
   CONSTRAINT` на SQLite не существует, так что внешние ключи можно написать
   только в `CREATE TABLE`.
...
сразу конечная схема: бордовые таблицы, наши таблицы и внешние ключи между ними.
...
specification without a key length». У нас так объявлен каждый ключ. Домашний
стиль доски — `VARCHAR(36)` для id; короткие технические строки (`node_id`,
`kind`, `status`, `mode`) — `VARCHAR(n)`, свободный текст (`title`, `summary`,
`said`, `detail`, `payload_json`) остаётся `TEXT`.
...
**`INTEGER` для времени — сломается на MySQL сразу.** Там это 32 бита, а unix-мс
сегодня ≈ 1.7·10¹², то есть переполнение в тысячу раз. У нас `INTEGER NOT NULL`
почти в каждой таблице и работает только потому, что в SQLite INTEGER — до
восьми байт. Везде `BIGINT`, как у доски.

**`AUTOINCREMENT` пишется по-разному, а у доски его нет вовсе.** У нас три
таких ключа: `session_event.id`, `flow_event.id`, `source_event.id`. Спеллинги:
SQLite `INTEGER PRIMARY KEY AUTOINCREMENT`, MySQL `BIGINT NOT NULL
AUTO_INCREMENT`, Postgres `BIGSERIAL`. Но в бордовой схеме автоинкремента нет ни
одного — id везде строки, сгенерированные приложением. Дешевле пойти домашним
путём и убрать три `{{if}}` совсем; цена в том, что порядок в журнале держится
на `id` (`ORDER BY id`, `idx_flow_event_card(card_id, id)`), так что понадобится
явный `seq` рядом с `created_at` — в пределах миллисекунды события бывают.
