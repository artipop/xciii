...
появляться в списке доски разработки. `Global` — явное «на всех досках».
...
**TypeScript** — пакет `@xciii/source-sdk` в npm, запускаемый через `npx`:
...
отсутствовать. Значит единственная защита — токен; слоя аутентификации
пользователя под ним нет.
...
Сейчас телефон подключиться не сможет: в десктопной сборке `frontdoor.go`
слушает `127.0.0.1` на случайном порту, меняющемся при каждом запуске. Поэтому у
ingest предусмотрен отдельный слушатель `sourcesListen`, по умолчанию
выключенный, обслуживающий только этот путь и всегда требующий токен. В
server-сборке он не нужен — адрес уже опубликован.

Привязка устройства: приложение показывает QR с `{url, token, source}`. **Каждое
устройство — отдельная запись реестра**, чтобы отзыв одного телефона не отключал
остальные и чтобы в журнале было видно отправителя.

Клиент в этом заходе не пишется — фиксируется протокол. С ним уже работают
`curl`, iOS Shortcuts и Tasker.
...
Сбор выполняет плагин, по одному на платформу. Стоимость различается: на Linux —
DBus, дёшево; на Windows — `UserNotificationListener` с согласием пользователя;
на macOS программного доступа к уведомлениям нет, только обходные пути через
хелпер с полным доступом к диску, и возможно, что на macOS источник останется
только телефонным. Это дополнительный довод за плагины: три платформы — три
разные программы, которым нечего делать в одном бинаре с доской.
...
**Колонка «Входящие»** приезжает в шаблоне доски, а не создаётся из Go: у
шаблонов есть версия и маркер (`TemplateVersion`, `xciiiTemplate`), то есть
готовый способ доставить изменение на уже созданные доски.
...
Токены в конфигурации не хранятся. В `SourceEntry` лежит `secretRef` — имя
...
`0600`, для headless-Linux, где Secret Service отсутствует.
...
- `internal/sources/plugin` — протокол: типы сообщений, клиент подпроцесса и
  отражающий интерфейс для встроенных источников.
- `sources/sdk` — библиотека для авторов Go-плагинов; вне `internal`, поскольку
  импортируется снаружи.
...
Экран привязки телефона (QR и токен) — секция того же диалога.
...
`needs-reauth` — ровно этот случай.
...
4. UI, привязка телефона, шаг мастера. С этого момента подсистема применима.
