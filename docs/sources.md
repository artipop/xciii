...
1. **Расширение общего доступа** — `.appex` внутри `XCIII.app`. Оно ничего не
   знает про доски и не ходит в сеть: берёт URL и заголовок из
   `NSExtensionItem` и открывает `xciii://share?url=…&title=…`. Так сделано не
   для простоты, а потому что расширение — отдельный песочный процесс: чтобы
   показать список досок, ему понадобился бы адрес и токен приложения, то есть
   общий контейнер (App Group) и подпись командой разработчика. Открыть URL
   умеет любое расширение и без всего этого.
