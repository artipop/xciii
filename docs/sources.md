...
уведомления с телефона. Писался он до реализации и остаётся местом, где записаны
решения, а не пересказом кода: §§1–23 — замысел, §§24–28 дописаны по ходу и
описывают то, что уже работает.
...
шаблон `home-chores` (`internal/boardadapter/templates/`), и на нём подсистема
нужна без пояснений: пришло «Доставка приедет завтра с 10 до 14» — появилась
карточка.
...
Структура повторяет `WorkdirEntry` (`internal/acp/config.go`): реестр хранится
...
CRUD — по образцу `agents.go` и `workdirs.go`: снимок под `cfgMu`, `Validate` с
...
Границу видно на карте: слева — четыре разных способа добыть элемент, справа —
один и тот же путь после него.

```mermaid
flowchart LR
    subgraph outside["Чужой код и внешний мир"]
        P1["Плагин<br/>sources/protocol"]
        P2["MCP-сервер<br/>по манифесту, §25"]
        P3["MCP-сервер<br/>через агента, §26"]
        P4["Скрипт, телефон,<br/>«Поделиться» — §13, §24"]
    end
    subgraph app["Приложение"]
        R["Раннер:<br/>расписание, повторы,<br/>токен, состояние"]
        M["Manager.Deliver"]
        D{"Дедупликация:<br/>source, external_id, version"}
        RU["Правила"]
        W["boardadapter.Writer"]
    end
    B[("Доска")]
    P1 --> R
    P2 --> R
    P3 --> R
    P4 -- "POST /sources/ingest/имя" --> M
    R --> M
    M --> D
    D --> RU
    RU --> W
...
Жизнь плагина целиком:

```mermaid
sequenceDiagram
    participant A as Приложение
    participant P as Плагин
    A->>P: initialize — config, credentials
    P-->>A: protocolVersion, capabilities
    loop пока источник включён и умеет poll
        A->>P: poll — cursor
        P-->>A: items, cursor, retryAfterSeconds
        Note over A: дальше конвейер §6
    end
    P->>A: items — push-уведомление, тот же код
    P->>A: log, needsReauth
    A->>P: credentials/update — токен обновлён
    A->>P: shutdown
```
...
```mermaid
flowchart TD
    S1["Скрипт на этой машине,<br/>агент из §26"] --> L["Петлевая дверь<br/>127.0.0.1, случайный порт"]
    S2["Телефон, mobile/,<br/>всё, что вне машины"] --> T["tailnet-дверь<br/>TLS, WhoIs"]
    T --> WHO{"Логин в tailnet<br/>разрешён?"}
    WHO -- нет --> X403["403 — до маршрута<br/>дело не доходит"]
    WHO -- да --> TOK
    L --> TOK{"Токен источника<br/>совпал с tokenHash?"}
    TOK -- нет --> X401["401"]
    TOK -- да --> OK["Item — конвейер §6"]
```
...
причинам, записанным в [CLAUDE.md](../CLAUDE.md): порт доступен всему, что может до
...
**Свойство доски `xciiiSources`** рядом с `xciiiColumns` и `xciiiFlows`
...
действует реестр. `home-chores` запрашивает телефон, `developer-tasks` — нет.

Префикс `xciii`, а не `acp`: подсистема источников к агентской интеграции не
относится и обязана работать при выключенной (§18), поэтому ключ на доске,
которую наполняет телефон, не может называться агентским. Тот же префикс несёт
маркер шаблона `xciiiTemplate` (`internal/boardadapter/templates.go`), и по той
же причине. Ранее здесь стояло `acpSources` — ошибка, реализации у неё не было.
...
существующих досках делает `arrangeKanbans` при заведении инбокса.

**Рядом с ней заводится вторая — «Мои задачи».** Кнопка «Создать» во «Входящих»
никуда не делась, но задача, которую человек написал сам, — не «пришло и никто
не смотрел», и автоматике эти два случая различать надо. Механика — фильтр
вида: он пропускает обе колонки и называет «Мои задачи» **первой**, а первое
значение условия `includes` — это то, чем становится карточка, созданная в этом
виде (`CardFilter.propertyThatMeetsFilterClause`). С канбана эта колонка, в
отличие от «Входящих», не убирается и стоит первой: из неё карточка уезжает
перетаскиванием, а колонки самого инбокса не перетаскиваются.

**Человек же видит инбокс отдельным видом доски, а не колонкой.** Сайдбар и так
перечисляет виды доски подпунктами под ней, рядом с календарным и табличным, —
то есть инбокс оказывается там, где человек ищет часть доски, и непрочитанное не
лежит посреди работы.

Вид — **канбан**, сгруппированный по автору: колонка на источник, и это тот
вопрос, который человек и задаёт инбоксу («что принесла почта, что Kaiten»).
Перетаскивать карточку между этими колонками нельзя — колонка здесь факт о
прошлом, а не место, куда карточку кладут (`kanban.tsx`, `columnsAreFacts`).

Вид приезжает **в шаблоне**, вместе с колонкой: доска, созданная из шаблона,
показывает «Входящие» с первой минуты, потому что «создал доску — ищу инбокс —
его нет» читается как отсутствующая функция, а не как «источник ещё не заведён».
Помечен он на минуту позже канбана: доска открывается на самом старом виде, а
при равном времени решает id — и с одинаковой отметкой каждая новая доска
открывалась бы на пустом инбоксе.

Для всего остального — доска старше этого решения, доска, созданную пустой, —
вид и колонку доводит `EnsureInbox` при заведении источника и перед каждой
записью. На доске без источника, созданной не из шаблона, инбокса нет: туда
ничего не приходит.

Что именно заводится на доске, и кто это делает:

```mermaid
flowchart LR
    T["Шаблон доски<br/>templates/*.jsonl"] --> C
    T --> V
    E["Заведение источника<br/>и каждая запись"] --> EI["EnsureInbox"]
    EI --> C["Опция «Входящие»<br/>в свойстве колонок"]
    EI --> V["Вид «Входящие»:<br/>канбан, группировка по автору"]
    EI --> P["Свойство «Автор»<br/>createdBy"]
    EI --> U["Аккаунт источника<br/>EnsureSourceUser"]
    C -.-> HID["Скрыта на канбане:<br/>hiddenOptionIds"]
    U -.-> V
    P -.-> V
```

Пунктиром — то, из чего складывается вид: колонки в нём берутся из автора
карточки, то есть из аккаунта источника, а не из свойства, которое пришлось бы
заводить и синхронизировать.
...
реестром. Ссылка остаётся: это другой факт. Но и она едет не именем — конвейер
кладёт её в `CardSpec.URL`, а `boardadapter` находит на доске свойство *типа*
`url` и заводит его, если такого нет. Имя «Ссылка» — то, как оно называется при
создании, и никогда не то, по чему его ищут: доска на другом языке, или поле,
которое переименовали, теряли ссылку молча.
...
Вид узнаётся по тому, что он показывает: по фильтру на опцию «Входящих», а
заголовок — только запасной вариант. Переименовать вид человек вправе, и раньше
это стоило второго инбокса при следующей же карточке.
...
```mermaid
flowchart TD
    subgraph in["Входящий: приложение только проверяет"]
        T1["Токен ingest"] --> H["SHA-256"]
        H --> HS["tokenHash в sources.json —<br/>сам токен показан один раз"]
    end
    subgraph out["Исходящий: приложение предъявляет чужому сервису"]
        T2["Токен сервиса, OAuth"] --> CH["secrets.Chain,<br/>читается по порядку"]
        CH --> EV["Окружение XCIII_SECRET_*<br/>выигрывает намеренно"]
        CH --> KC["Keychain ОС —<br/>zalando/go-keyring"]
        CH --> FS["secrets.json 0600 —<br/>только там, где keychain нет"]
    end
```
...
- **Свойство ссылки (`url`)** — только добавление, без удаления, как в
  синхронизации реестра проектов: на опции могут ссылаться карточки. Ищется по
  типу, не по имени (`propertyOfType`); свойство «Источник» (select) не нужно —
  автор карточки отвечает на тот же вопрос.
...
sequenceDiagram
    ...
    participant X as Расширение .appex
    participant A as XCIII
    participant W as Окно на /share
    participant M as Источник «Поделиться»
    S->>X: NSExtensionItem — url, title
    X->>A: xciii://share?url=...&title=...
    A->>W: маленькое окно на своей же странице
    W->>A: ListBoards — какие доски есть
    W->>A: ShareItem — доска, заголовок, ссылка, заметка
    A->>M: EnsureSource и Item в конвейер
    M-->>W: «во Входящих» или «уже приносили»
```
...
```mermaid
flowchart TD
    MF["Манифест, kind: mcp"] --> DP["dialPlugin — единственная развилка"]
    DP --> CL["MCP-клиент: initialize, tools/list"]
    CL --> CHK{"Инструмент mcp.tool<br/>у сервера есть?"}
    CHK -- нет --> ERR["Ошибка при подключении<br/>со списком того, что есть"]
    CHK -- да --> CALL["tools/call —<br/>arguments по шаблону из записи"]
    CALL --> ISERR{"isError<br/>в успешном ответе?"}
    ISERR -- да --> FAIL["Отказ инструмента<br/>в журнал источника"]
    ISERR -- нет --> ROWS["itemsAt: строки ответа"]
    ROWS --> IT["item: шаблоны по строке —<br/>Item"]
    IT --> PIPE["Конвейер §6"]
```
...
Манифесты берутся из двух мест. Первое — **вкомпилированные**
(`internal/sources/manifests`, `go:embed`): сюда попадает сервис, чей
MCP-сервер везёт само приложение, — тогда манифест пишет `$self` вместо пути,
и `Argv` подставляет путь к приложению на старте. Так сделан Kaiten
(`internal/sources/kaiten`, `xciii mcp kaiten`): его добавляют из диалога, и
ставить нечего. Второе — **`<dataDir>/sources/manifests/*.json`**, для сервера,
который живёт на этой машине отдельно: такой манифест называет абсолютный путь
(`dir`), то есть возить его в репозитории нечего. Файл из каталога данных
перебивает вкомпилированный с тем же именем. Битый файл — сообщение в журнале и
пропуск: приложение обязано подняться, чтобы было чем сказать, что с ним не
так.
...
```mermaid
sequenceDiagram
    participant R as Раннер источника
    participant AG as Агент
    participant SRV as MCP сервиса
    participant IN as mcp inbox — наш
    participant AP as Ingest приложения
    R->>R: IssueToken — новый токен на один ход
    R->>AG: RunForSource — промпт и два MCP-сервера
    loop на каждую находку
        AG->>SRV: инструменты сервиса
        SRV-->>AG: что нашлось
        AG->>IN: file_item
        IN->>AP: POST /sources/ingest/имя
        AP-->>IN: «создана карточка» или «уже приносили»
    end
    AG-->>R: ход закончен, таймаут 10 минут
```
...
## 28. Что переживает перезапуск

Состояние подсистемы лежит в четырёх местах, и делятся они по смыслу: файл —
то, что настроил человек; база — то, что уже случилось; keychain — то, что
нельзя показывать; память — то, что дешевле узнать заново, чем хранить.

```mermaid
flowchart TB
    subgraph disk["Каталог данных"]
        CFG["sources.json, 0600:<br/>записи источников, правила,<br/>tokenHash, вставленные манифесты"]
        DB["sources.db:<br/>source_item — что уже приносили,<br/>source_event — журнал"]
        MAN["sources/manifests/*.json:<br/>читаются заново при каждом старте"]
        FS["secrets.json, 0600 —<br/>только там, где keychain нет"]
    end
    subgraph os["Хранилище ОС"]
        KC["Keychain, Credential Manager,<br/>Secret Service — служба XCIII"]
    end
    subgraph brd["База доски"]
        B["Карточки, опция и вид «Входящие»,<br/>свойство «Автор», аккаунт источника"]
    end
    subgraph mem["Память процесса — теряется"]
        ST["Status: последний опрос,<br/>последняя ошибка, состояние"]
        CU["Курсор плагина"]
        PR["Подпроцессы плагинов,<br/>goroutine на источник"]
        DL["delivery: как доска зовёт<br/>свойство колонок, на одну пачку"]
    end
    subgraph bin["Бинарь"]
        EM["Встроенные манифесты,<br/>go:embed — Kaiten"]
    end
```

| Что | Где | Переживает перезапуск |
|---|---|---|
| Записи источников, правила, интервалы, `pickBoard` | `<dataDir>/sources.json` | да |
| Хэш входящего токена | там же, `tokenHash` | да; сам токен — нет, он показан один раз |
| Манифесты, вставленные человеком | там же, `plugins` | да |
| Манифесты этой машины | `<dataDir>/sources/manifests/*.json` | да, перечитываются на старте |
| Встроенные манифесты | бинарь, `go:embed` | да, они его часть |
| Что уже приносили: `(source, external_id, version)` → карточка | `sources.db`, `source_item` | да |
| Журнал: элемент, правило, карточка, ошибка | `sources.db`, `source_event` | да |
| Токен, который приложение **предъявляет** сервису | keychain ОС, иначе `secrets.json`; окружение перебивает оба | да |
| Карточки, колонка и вид «Входящие», аккаунт источника | база доски | да |
| Курсор плагина | память раннера | **нет** |
| Состояние источника: последний опрос, ошибка, `needs-reauth` | память, `Status` | **нет** |
| Процессы плагинов | подпроцессы под контекстом приложения | **нет** |
| `delivery` — чем доска зовёт свойство колонок | память, на одну пачку | **нет** |

Три следствия, которые видно только из этой таблицы.

**Курсор в памяти — это осознанная плата за одну гарантию.** После перезапуска
плагин опрашивается без курсора, то есть отдаёт всё, что видит; вторых карточек
из этого не выходит, потому что от них защищает не курсор, а пара
`(external_id, version)` в `source_item`. Курсор экономит трафик, а не
правильность, и хранить его в файле пришлось бы согласованно с базой — ради
экономии, которая случается раз в запуск.

**Состояние источника пустое до первого опроса.** Диалог показывает «ещё не
опрашивался», а не «сломан», и это правда: раннер поднимается вместе с
приложением и первый опрос делает сразу, не дожидаясь расписания.

**Токен выдаётся один раз и хранится хэшем**, поэтому «показать токен ещё раз»
невозможно ни в каком интерфейсе — есть только «выдать новый», и он отменяет
предыдущий. Агентский источник пользуется этим по-своему: `IssueToken` на
каждый ход, потому что открытым текстом токен уходит в чужой процесс.

## 29. Плагин IMAP

Первый плагин, говорящий на самом протоколе §9 (а не манифестом-мостом, как
Kaiten, §25): почтовый ящик по IMAP. Он же — доказательство того, что §9–§11
достаточно, чтобы написать полноценный источник без единой правки в остальной
подсистеме.

Плагин встроен в бинарь приложения и живёт в `internal/sources/imapsource`,
запускается как `xciii sourceplugin imap` — тот же приём, что `xciii mcp
kaiten` (§25), только для другого вида манифеста: `command: "$self"`,
`args: ["sourceplugin", "imap"]`, без `kind` (по умолчанию — сам протокол).
Диспетчер `sourceplugin.go` — точная копия `maybeRunMCP`, вызывается из
`main()` рядом с ним.

Учётные данные — логин и пароль (обычно пароль приложения, если у почты
включена двухфакторная аутентификация) — идут через существующий механизм
`auth: {"type": "token"}`, тот же, что у Kaiten: единственное отличие в том,
что здесь «токен» — это пароль почтового ящика, а не токен API. Хост, порт,
логин и папка — обычные поля манифеста (`internal/sources/manifests/
imap.json`).

**Курсор — пара (UIDVALIDITY, последний увиденный UID), и первый опрос ничего
не возвращает.** Решение сознательное: только что заведённый источник не
тянет всё, что уже лежит в ящике, — он слушает с этого момента. Первый `poll`
запоминает `UIDNEXT` как границу и не отдаёт ни одного элемента; каждый
следующий ищет `UID больше границы`. Если `UIDVALIDITY` изменился — сервер
пересоздал или перенумеровал ящик, — граница сбрасывается ровно так же, как
при первом опросе, а не читает старый диапазон UID как новый: чужая
нумерация. `ExternalID` — заголовок `Message-ID` письма (без угловых
скобок), а не UID: это то, что письмо принесло с собой, и не меняется, если
письмо увидели через другой ящик. `Version` равен `ExternalID` — у письма нет
состояния, которое меняется, поэтому политика «comment» на этом источнике
никогда не сработает, и это не недостаток.

`Body` — часть `text/plain`, а если её нет — `text/html` с вырезанными
тегами: `Item.Body` становится текстом карточки, а не HTML-разметкой.

Ошибки: неверный пароль — `needs_reauth`; сеть, обрыв соединения — `retryable`;
несуществующая папка — `bad_config` на поле `mailbox`, которое диалог
источника подсвечивает без дополнительного кода.

**Второй этап — вход через Gmail и Яндекс без пароля приложения (OAuth) — не
сделан.** Что для него нужно и почему это отдельная работа, записано в
