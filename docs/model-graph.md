...
### 1. ~~«Какое свойство — колонки» ищется по имени~~ — почти закрыто
...
Что осталось: **отрицательный** ответ. Чтобы сказать «карточка отсюда ушла»,
надо знать, какая из её опций — колонка, а id свойства не записан ни у маршрута,
ни у стадии — только имя. Так что переименование свойства всё ещё может стоить
ответа «ушла», и это безопасная сторона: остаётся запись маршрута. Полностью
закрывает `xciiiColumnProperty` на доске — id свойства рядом с
`xciiiProjectProperty` и `xciiiBranchProperty`; конфиг тогда становится дефолтом
для доски, которая ничего не записала.
...
Реестр папок уже по id, а `workdir_claim.workdir_path` — путь на диске.
Переместили папку — claim осиротел: карточка считается начатой (ветка на ней), а
копию никто не найдёт. Плюс это единственное, что мешает записи реестра
перестать быть папкой: у гугл-диска или ssh-машины «пути» в этом смысле нет.

Чинится сменой ключа на `WorkdirEntry.ID` — миграция одной таблицы. Колонка уже
переименована из `workdir` в `workdir_path`, чтобы имя не врало до тех пор.
...
**Первый шаг сделан** (2026-08-17): таблицы в одной базе, ключи написаны в
`CREATE TABLE`. Проверка ими ещё не включена — это шаг 4, — но написать их можно
было только сейчас: `ALTER TABLE ADD CONSTRAINT` на SQLite не существует.
