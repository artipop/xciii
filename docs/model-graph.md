...
Из него следует переезд на два хранилища вместо четырёх: реестры перестают быть
JSON и становятся таблицами с внешними ключами, `config.json` перестаёт называть
колонки чужих досок, `sources.db` въезжает в ту же базу. Четыре работы, первые
две независимы, шесть противоречий из девяти закрываются ими — целевая схема,
шаги, миграции и чем каждый шаг проверяется: **`docs/store-plan.md`**.

Не входит туда — и это отдельное решение, а не недоделка — перенос наших таблиц
в базу доски. `internal/boardadapter` единственный импортирует обе стороны, и
этот шов существует затем, чтобы доску можно было заменить; настоящий FK из
наших таблиц в `blocks(id)` его сваривает. Поэтому одна база **наша**, а доска
остаётся чужой системой по id — это же то, что позволяет карточке переезжать
между машинами.

Пункты 4 и 5 живут целиком внутри JSON доски и хранилищ не касаются вовсе.
Пункт 7 идентификаторами не чинится: чтобы продолжить работу на второй машине,
там должна оказаться та же папка.
