...
Каждое удаление записывается в `docs/fork-shrink-removed.md` — одна строка на
подсистему и коммит, которым её возвращают. Не летопись ради летописи:
«мертво» — суждение о сегодняшнем дне, решения слоя 0 уже один раз
перевернули план, и журнал есть способ передумать дёшево.
