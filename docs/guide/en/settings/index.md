...
| `worktreeMode` | what to do in a repository on a board that was never asked: `always` — a copy of its own per card, `never` — a branch in the folder itself. The board's own answer (board menu → "Folders…") wins — see [Folders and branches](../folders.md) |
